...
def categorize():
    ...
    for folder in path:
        if os.path.isdir(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/" + folder)):
            ...
            for file in files:
                if file.endswith('.rfd') and not file.startswith('._') and not os.path.isdir(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/" + folder + '/' + file)):
                    ...
                    hi = header.rfgi.rfAcqGain
                    yo = header.rfsd[0].analogGain
                    hi = header.rfsd[0].txFocusRangeCm
                    yo = header.rfsd[0].rxFocusRangeCm
                    header = [int(header.rfbd.numSamplesPerVector-header.strf.vectorHeaderLengthBytes/2), int(header.csh0.numVectorsPerStreamFrame), header.rfgi.rfAcqGain, header.rfsd[0].txFocusRangeCm]
             


                    hi = True
                    for i in range(presetNum):
                        curHeader = np.loadtxt(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/Phantom/Preset_" + str(i+1) + "/header"))
                        # curHeader = samplesAr[i]
                        if all(header[i] == curHeader[i] for i in range(len(curHeader))):
                            try:
                                os.mkdir(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/" + folder + "/Preset_" + str(i+1)))
                            except:
                                pass
                            os.rename(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/" + folder + '/' + file), str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/" + folder + "/Preset_" + str(i+1) + "/" + file))
                            hi = False
                            break

                    if hi:
                        try:
                            os.mkdir(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/Phantom/Preset_" + str(presetNum+1)))
                            os.mkdir(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/" + folder + "/Preset_" + str(presetNum+1)))
                        except:
                            pass
                        np.savetxt(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/Phantom/Preset_" + str(presetNum+1) + "/header"), header)
                        os.rename(str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/" + folder + '/' + file), str("/Volumes/CREST Data/David_S_Data/Thyroid_Data_RFD/" + folder + "/Preset_" + str(presetNum+1) + "/" + file))
