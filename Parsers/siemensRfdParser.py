...
def getImage(filename, filedirectory, refname, refdirectory):
    ...
    bmode = np.clip(ImgData.bMode, ImgInfo.clipFact*ImgInfo.maxval-ImgInfo.dynRange, ImgInfo.clipFact*ImgInfo.maxval)
...
def getData(Files, RefFiles, AnalysisParams):
    ...
    [imgData, imgInfo] = readFileImg(imgInfo, AnalysisParams.frame, AnalysisParams.focus)

    refInfo = readFileInfo(RefFiles.name, RefFiles.directory)
    [refData, refInfo] = readFileImg(refInfo, AnalysisParams.frame, AnalysisParams.focus)
    
    return imgInfo, refInfo, imgData, refData

def readFileImg(Info, frame, focus):
    file_obj = open(str(Info.filepath+Info.filename), 'rb')
    FileHeader = readHeader(str(Info.filepath+Info.filename))
    [data, _]  = extractFrameData(file_obj, FileHeader, frame)
    ...
    Info.width = Info.lateralRes*bmode.shape[1]
    Info.axialRes = 1500000/40000000
    Info.height = Info.axialRes*bmode.shape[0]
...
def splitData(imgData, focus):
    newData = np.zeros((imgData.shape[0], 256))
    if focus == 1:
...
def readFileInfo(filename, filepath):
    ...
    info.height = ((2*1500000)/40000000) * info.samples
    info.pitch = 0
    info.dynRange = 55
    info.yOffset = 0
    info.vOffset = 0
    info.lowBandFreq = 5000000
    info.upBandFreq = 15000000
    info.gain = 0
    info.rxGain = 0
    info.userGain = 0
    info.txPower = 0
...
if __name__ == "__main__":
    getImage('uri_SpV2232_VpF512_FpA90_20200221124749.rfd', '/Users/davidspector/Home/Stanford/Misc Data/', 'uri_SpV2232_VpF512_FpA90_20210129103614_orig.rfd','/Users/davidspector/Home/Stanford/Misc Data/')